...
class BuildCommand extends PlayerCommand("build") {
  ...
  def commandPart2: CommandPart = (sender, cmd, name, args) => {
  ...
  override def commandparts: List[CommandPart] = List(permissionpart, commandPart, playerpart, commandPart2)
}
